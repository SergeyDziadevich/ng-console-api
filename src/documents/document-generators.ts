...
export async function generateB2BContractPlPdf(
  ...
  const getLines = (text: string, font: any, size: number): string[] => {
  ...
  const drawParagraph = (text: string, isBold = false, size = 10, lineSpacing = 14) => {
  ...
  const dateText = clean(`Zawarta w dniu ${new Date().toLocaleDateString('pl-PL')} r. w Warszawie, pomiędzy:`);
  ...
  // Parties
  const clientInfo = data.clientName || 'Firma A Sp. z o.o. z siedzibą w Warszawie, ul. Przykładowa 1, 00-001 Warszawa, NIP: 1234567890, reprezentowana przez: Jana Kowalskiego – Prezesa Zarządu';
  ...
  const contractorInfo = data.contractorName || 'Firma B Jan Nowak z siedzibą w Krakowie, ul. Testowa 2, 30-002 Kraków, NIP: 0987654321';
  const contractorText = `Wykonawcą: ${contractorInfo}`;
  drawParagraph(clean(contractorText), false, 10, 14);

  // Sections
  const services = data.servicesDescription || 'programowaniu oraz utrzymaniu systemów informatycznych Zamawiającego';
  ...
  const fee = data.monthlyFee || '[kwota] PLN netto za każdą przepracowaną godzinę';
...
export async function generateMsaPdf(data: MsaData): Promise<Uint8Array> {
  ...
  drawParagraph(`This Master Services Agreement ("Agreement") is entered into by and between ${provider} ("Provider") and ${client} ("Client").`, false, 10, 14);

  // Sections
  const services = data.servicesDescription || 'general software engineering, DevOps, IT consulting, and project management services';
